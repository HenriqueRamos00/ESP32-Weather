...
async def get_api_keys(
    ...
    """Get all API keys."""
    api_keys = await ApiKeyService.get_all(db, skip=skip, limit=limit)
    total = await ApiKeyService.count(db)
...
async def get_device_api_keys(
    ...
    api_keys = await ApiKeyService.get_by_device_id(db, device_id)
    total = await ApiKeyService.count_by_device(db, device_id)
    return {"api_keys": api_keys, "total": total}
...
async def create_api_key(api_key_in: ApiKeyCreate, db: AsyncSessionDep) -> Any:
    ...
    device = await DeviceService.get_by_id(db, api_key_in.device_id)
    if not device:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Device not found",
...
async def delete_api_key(key_id: int, db: AsyncSessionDep) -> None:
    """Delete an API key."""
    success = await ApiKeyService.delete(db, key_id)
    if not success:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="API key not found",
